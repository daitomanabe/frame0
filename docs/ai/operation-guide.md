...
- `graph` shows nodes, references, validation errors, cycles, and topological
  order.
- `scene controls` extracts UI controls, preset coverage, shader uniforms, and
  artifact groups for parameter-heavy scenes.
...
- `launch.json`: structured launch metadata, including graph, adapter status,
  and control-surface summary
...
- [ ] Inspect controls with `frame0 scene controls <scene> --json` when the
      scene contains presets, UI controls, shader uniforms, or artifact groups.
